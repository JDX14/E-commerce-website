...
$query = "SELECT * FROM products WHERE product_category = 'disposable' LIMIT $start, $productsPerPage";
$result = mysqli_query($conn, $query);

// Calculate total number of products
$totalProductsQuery = "SELECT COUNT(*) as total FROM products WHERE product_category = 'disposable'";
...
<head>
    ...
    <title>Disposable Products 1</title>
...
<body>
    ...
    <!-- ALL PRODUCTS -->
  <!-- Featured Products -->
  <section class="featured disposable-products">
        <h2 class="title">Disposable Products</h2>
    ...
    // Previous page link
    if ($current_page > 1) {
        echo "<a href='disposable.php?page=" . ($current_page - 1) . "'>&laquo; Previous</a>";
    }
    ...
    for ($i = 1; $i <= $totalPages; $i++) {
        if ($i == $current_page) {
            ...
            echo "<a href='disposable.php?page=$i'>$i</a>";
        }
    }

    // Next page link
    if ($current_page < $totalPages) {
        echo "<a href='disposable.php?page=" . ($current_page + 1) . "'>Next &raquo;</a>";
    }
    ?>
</div>
